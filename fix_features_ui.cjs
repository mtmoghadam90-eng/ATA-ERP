...
content = content.replace(oldFeatureInput, newFeatureInput);

const oldOptionsUI = `                          {feature.options.map((opt, oIndex) => (
...
const newOptionsUI = `                          {feature.options.map((opt, oIndex) => (
...
content = content.replace(oldOptionsUI, newOptionsUI);

fs.writeFileSync('src/components/ProductsView.tsx', content);
console.log('Fixed features UI');
