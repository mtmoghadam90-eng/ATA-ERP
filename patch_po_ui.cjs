...
const supplierFieldMatch = `                {/* Supplier Field */}
                <div className="space-y-1.5 flex-1 min-w-0">
                  <label className="text-xs font-semibold text-slate-500">تأمین‌کننده *</label>
                  <div className="flex gap-1.5 items-center w-full min-w-0">
                    ...
                    <button
                      ...
                      className="p-2.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition shrink-0"
                      title="ثبت سریع تأمین‌کننده جدید"
                    >
                      <Plus size={16} />
                    </button>
                  </div>
...
const supplierInquiryField = `
                ...
                <div className="space-y-1.5 flex-1 min-w-0 sm:col-span-2 mt-4 bg-amber-50 p-3 rounded-lg border border-amber-100">
                  <label className="text-xs font-semibold text-amber-800">تکمیل خودکار از طریق استعلام‌های برنده (اختیاری)</label>
                  <select
                    className="w-full border border-amber-200 rounded-lg px-3 py-2 text-sm text-right bg-white focus:border-amber-400 outline-none"
                    onChange={(e) => {
                      ...
                      if (inq) {
                        ...
                        const poItems = inq.items.map((inqItem, idx) => ({
                          ...
                          supplierNotes: inqItem.notes || \`زمان تحویل: \${inqItem.deliveryTime || ''}\`
                        }));
                        setItems(poItems);
...
content = content.replace(supplierFieldMatch, supplierFieldMatch + supplierInquiryField);
